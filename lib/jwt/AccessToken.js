 ...
 * @constructor
 * @param {object} options - ...
 * @param {string} options.playbackGrant - the stringified json with the playback grant
 */
function PlayerGrant(options) {
  options = options || {};
  this.playbackGrant = options.playbackGrant;
}

_.extend(PlayerGrant.prototype, {
  key: 'player',
  toPayload: function() {
    var grant = '';
    if (this.playbackGrant) { grant = this.playbackGrant; }
    return grant;
  }
});
...
AccessToken.PlayerGrant = PlayerGrant;
